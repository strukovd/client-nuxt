<template>
  <section class="m-page-view">
    <header>
      <AppToolbar class="wrapper" @selectedCity="()=>{}"/>
    </header>
    <section v-if="loading" class="preloader">
      <v-skeleton-loader style="margin-top:4em;"
          type="image" width="90vw"/>
      <v-skeleton-loader style="margin-top:1em;"
          type="image" width="90vw" height="26px"/>
          <v-skeleton-loader style="margin-top:1.5em;"
          type="paragraph" width="90vw"/>
      <v-skeleton-loader v-for="i in 10" style="margin-top:.1em;" :key="i"
          type="text" width="90vw"/>
    </section>
    <main v-else class="m-page-content">
      <section class="breadcrumbs">
        <BaseBreadcrumbs :breadcrumbs="[{href: '/', title: 'Главная'}, {href: '/establishments', title: 'Заведения'}, {href: '', title: model?.name}]"/>
      </section>
      <section class="slider" style="border-radius:20px; overflow:hidden; position:relative;">
        <!--
        <VueperSlides :bullets="images.length <= 4" autoplay :infinite="true" :arrows="images.length > 4">
          <VueperSlide v-for="(slide,index) of images">
            <template #content>
              <v-img width="100%" height="100%" :src="`https://files.kipish.kg/${slide.minioBucket}/min-${slide.minioPath}`"></v-img>
            </template>
          </VueperSlide>
          <template #bullet="{ active, slideIndex, index }">
            <div>
              <v-img v-if="active" :width="true <= 400 ? '50' : '64'" src="/static/images/slide-active.svg"/>
              <v-img v-if="!active" :width="true <= 400 ? '50' : '64'" src="/static/images/slide-unactive.svg"/>
            </div>
          </template>
        </VueperSlides>
        -->
        ...
        <!-- <aside style="position: absolute;bottom: 1em;right: 1em;display: flex;gap: 0.5em;font-size: 24px;line-height: 2em;color: #ffffff4d;">
          <div @click="$refs.slider.prev();" style="cursor:pointer;padding:0 .4em;" :style="{color: currentSlide !== 0 ? '#fff' : ''}">&lt;</div>
          ...
          <div @click="$refs.slider.next();" style="cursor:pointer;padding:0 .4em;" :style="{color: currentSlide !== (images.length - 1) ? '#fff' : ''}">&gt;</div>
        </aside> -->
      </section>

      <!-- Информация -->
      <section class="estab-info">
        <div class="ei-wrapper1">
          <div style="display:flex; flex-wrap:wrap;">
            <div v-if="Array.isArray(model.cousinTypes) && model.cousinTypes.length" style="flex-grow: 1;">
      ...
      <section v-if="model.name && model.description" class="estab-description">
        <div class="ed-caption">О заведении:</div>
      ...
      <section v-if="model.menuDescription" class="estab-menu">
        ...
        <div class="pictures">
          <img v-for="i of Math.min(3, cousinImages.length)" :key="i" :src="`https://files.kipish.kg/${cousinImages[i-1].minioBucket}/min-${cousinImages[i-1].minioPath}`">
        </div>
        <header>
      ...
      <section v-if="model.barDescription" class="estab-bar">
        ...
        <div class="pictures">
          <img v-for="i of Math.min(3, barImages.length)" :key="i" :src="`https://files.kipish.kg/${barImages[i-1].minioBucket}/min-${barImages[i-1].minioPath}`">
        </div>
        <header>
...
export default {
  ...
  computed: {
    ...mapGetters(['videoDomain', 'sourceId']),

    images() {
      if(!this.model?.images) return [];
      return this.model.images.filter(i => i.source === 'ESTABLISHMENT');
    },
    cousinImages() {
      if(!this.model?.images) return [];
      return this.model.images.filter(i => i.source === 'ESTABLISHMENT_COUSIN');
    },
    barImages() {
      if(!this.model?.images) return [];
      return this.model.images.filter(i => i.source === 'ESTABLISHMENT_BAR');
    }
  },
  ...
  methods: {
    fetch() {
      this.loading = true;
      this.$http2.get(`/establishments/${this.sourceId}`)
        .then(r => {
    ...
    fetchNearBy() {
      ...
      this.$http2.get(`/establishments/nearby/${this.sourceId}?limit=3`)
        .then(r => {
