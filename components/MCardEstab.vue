<template>
    <div class="card-estab-mobile" @click="$router.push('/establishment/' + (!!item?.url?.url ? item.url.url : item.id))"
    :style="{
      // width: '341px',
      // height: '324px',
      aspectRatio: '341 / 324',
      backgroundImage: `linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.5) 100%)`,
      // backgroundImage: `url(${item.coverImage ? item.coverImage : `https://files.kipish.kg/${item.coverPath}`})`,
  ...
  >
    <div class="card-inner" :style="{backgroundImage: `url(${item.coverImage ? item.coverImage : `https://files.kipish.kg/${item.coverPath}`})`}">
      <section class="front">
        <div class="estab-row">
          <div class="estab-type glass">{{ item.categories[0].nameRu }}</div>
          <div class="work-time glass">{{ item.hoursOfWork ?? `08:00 - 23:00` }}</div>
        </div>
  ...
  <!-- <div class="estabs_card_mobile">
    <div @click="$router.push('/establishment/' + (!!item?.url?.url ? item.url.url : item.id))" style="width: 100%;height: 100%;border-radius: 12px" class="cursor-pointer">
      <v-img lazy-src="/static/images/cover-2.jpg" height="330px"
      ...
      <footer style="background: #111111;border-radius: 0 0 12px 12px" class="pa-6 d-flex flex-column">
        ...
        <div class="d-flex flex-column my-2">
          ...
          <span class="text-18 white--text">{{ item.address ? truncateText(item.address, 25) : 'Уточняйте у администратора' }}</span>
        </div>
        <div class="d-flex flex-column">
